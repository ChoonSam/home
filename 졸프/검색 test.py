...
df2 = pd.read_csv("C:/Users/장준하/Desktop/형태소분석(NN)2.csv", encoding='utf-8')
...
indices = pd.Series(df2.index, index=df2['nn']).drop_duplicates()

# 영화의 제목을 입력받으면 코사인 유사도를 통해서 가장 유사도가 높은 상위 10개의 영화 목록 반환
def get_recommendations(nn, cosine_sim=cosine_sim):
    ...
    nn_indices = [i[0] for i in sim_scores]
    
    # 인덱스 정보를 통해 영화 제목 추출
    return df2['nn'].iloc[nn_indices]
